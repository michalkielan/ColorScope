...
class GraphGenerator:
  def __init__(self, ref_json_filename, cap_json_filename):
    self.__ref_color = ColorJsonParser(ref_json_filename)
    self.__cap_color = ColorJsonParser(cap_json_filename)
    if self.__ref_color.get()['format'] != 'hls' or self.__cap_color.get()['format'] != 'hls':
  ...
  def generate(self):
    ...
    color_meter = ColorMeter(self.__ref_color, self.__cap_color)
...
def main():
  ...
  if gen_graph_filenames != '':
    ...
    try:
      GraphGenerator.create(ref_json, cap_json)
